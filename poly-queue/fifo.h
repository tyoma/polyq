...
namespace pq
{
	...
	template <typename BaseT>
	inline fifo<BaseT>::fifo(count_t byte_size)
		: _write(0), _read(0), _start(0), _limit(0)
	{	grow_by(byte_size);	}

	template <typename BaseT>
	inline fifo<BaseT>::~fifo()
	{	destroy();	}

	template <typename BaseT>
	inline bool fifo<BaseT>::empty() const throw()
	{	return _read == _write;	}

	template <typename BaseT>
	template <typename T>
	inline void fifo<BaseT>::push_back(const T &value)
	{
	...
	template <typename BaseT>
	inline void fifo<BaseT>::pop_front() throw()
	{
	...
	template <typename BaseT>
	inline BaseT &fifo<BaseT>::front() throw()
	{	return _read->get<BaseT>();	}

	template <typename BaseT>
	inline void fifo<BaseT>::grow_by(count_t sz)
	{
	...
	template <typename BaseT>
	inline count_t fifo<BaseT>::capacity() const throw()
	{	return static_cast<count_t>(static_cast<uint8_t *>(_limit) - static_cast<uint8_t *>(_start));	}

	template <typename BaseT>
	inline void fifo<BaseT>::destroy() throw()
	{
		while (!empty())
			pop_front();
